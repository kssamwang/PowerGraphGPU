...
namespace graphlab {
    ...
    public:
        ...
        public:
            ...
            std::vector<int> src;
            std::vector<int> dest;
            std::vector<edge_data_type> data;
        };
    ...
    public:
        ...
        ****************************************************************************/
        ...
        const lvid_type maxlvid() const {
            if (edge_buffer.size()) {
                lvid_type max(0);
                        foreach(lvid_type i, edge_buffer.source_arr)max = std::max(max, i);
                        foreach(lvid_type i, edge_buffer.target_arr)max = std::max(max, i);
                return max;
            } else {
                return lvid_type(-1);
            }
    ...
    private:
        ...
        private:
            ...
            bool equal(const edge_iterator &other) const {
                ...
                switch (_type) {
                    case CSC:
                        return csc_iter == other.csc_iter;
                    case CSR:
                        return csr_iter == other.csr_iter;
                    default:
                        return true;
                }
            }
            ...
            ptrdiff_t distance_to(const edge_iterator &other) const {
                switch (_type) {
                    case CSC:
                        return other.csc_iter - csc_iter;
                    case CSR:
                        return other.csr_iter - csr_iter;
                    default:
                        return 0;
                }
            }
        ...
        private:
            edge_type make_value() const {
                switch (_type) {
                    ...
                    default:
                        return edge_type(lgraph_ref, -1, -1, -1);
                }
            }
